...
function AGIHandler(socket){
    ...
    parser.on('line', function(line){
        ...
        if(typeof command.cb == 'function'){
            ...
            let code = command.rx.substring(0, 3),
                result = null,
                data = null;
            ...
            const result_regex = /^(.*)\s(result=.?\d)\s(.*)$/;
            let parsed_answer = command.rx.match(result_regex);
            if (parsed_answer && parsed_answer[3]) {
                data = parsed_answer[3].substring(1, parsed_answer[3].length-1);
            }
